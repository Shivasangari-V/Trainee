...
public class CourseCRUDHandler implements CRUDHandler {
	@Override
	public JSONObject insertData(JSONObject request) throws Exception {
		JSONObject response = Validate.postValidatationConstraint("Course", request);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Course", constraint_fields, constraint_values)) {
				...
				DBUtil.insertData("Course", fields, values);
			}
		}
	...
	public JSONObject updateData(JSONObject request, JSONObject queryParam) throws Exception {
		JSONObject response = Validate.deleteValidation("Course", queryParam);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Course", constraint_keys, constraint_values) && request.length() != 0) {
				response = Validate.putValidation("Course", request);
				if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
					...
					DBUtil.updateData("Course", constraint_keys, constraint_values, keys, values);
				}
			} else {
				JSONObject details = new JSONObject();
	...
	public JSONObject deleteData(JSONObject request) throws Exception {
		JSONObject response = Validate.deleteValidation("Course", request);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			ArrayList<String> fields = new ArrayList<String>();
			ArrayList<Object> values = new ArrayList<Object>();
			fields.add("id");
			values.add(request.get("id"));
			if (DBUtil.isPresentData("Course", fields, values)) {
				DBUtil.deleteData("Course", request.getString("id").toString());
			} else {
				JSONObject details = new JSONObject();
				details.put("mismatch field", request);
	...
	public JSONObject getData(JSONObject request) throws Exception {
		...
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Course", constraint_keys, constraint_values)) {
				Model model = this.convertToPOJO(request);
				model = (Course) model;
				...
				JSONObject DBStatusResponse = DBUtil.getData("Course", "id",request.get("id"), getFields);
	...
	public Model convertToPOJO(JSONObject request) {
		...
		course.setId(request.optString("id"));
		course.setName(request.optString("name"));
		return course;
	}

...
public class DepartmentCRUDHandler implements CRUDHandler {
	@Override
	public JSONObject insertData(JSONObject request) throws Exception {
		JSONObject response = Validate.postValidatationConstraint("Department", request);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Department", constraint_fields, constraint_values)) {
				...
				DBUtil.insertData("Department", fields, values);
			}
		}
	...
	public JSONObject updateData(JSONObject request, JSONObject queryParam) throws Exception {
		JSONObject response = Validate.deleteValidation("Department", queryParam);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Department", constraint_keys, constraint_values) && request.length() != 0) {
				response = Validate.putValidation("Department", request);
				if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
					...
					DBUtil.updateData("Department", constraint_keys, constraint_values, keys, values);
				}
			} else {
				JSONObject details = new JSONObject();
	...
	public JSONObject deleteData(JSONObject request) throws Exception {
		JSONObject response = Validate.deleteValidation("Department", request);
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			ArrayList<String> fields = new ArrayList<String>();
			ArrayList<Object> values = new ArrayList<Object>();
			fields.add("id");
			values.add(request.get("id"));
			if (DBUtil.isPresentData("Department", fields, values)) {
				DBUtil.deleteData("Department", request.getString("id").toString());
			} else {
				JSONObject details = new JSONObject();
				details.put("mismatch field", request);
				JSONObject dbUpdateResponse = new ExceptionHandler(ExceptionHandler.ErrorCode.DATA_NOT_FOUND, "field not found", details, ExceptionHandler.Status.ERROR,206).getResponseJson();
	...
	public JSONObject getData(JSONObject request) throws Exception {
		...
		if (response.get("status") == ExceptionHandler.Status.SUCCESS) {
			...
			if (DBUtil.isPresentData("Department", constraint_keys, constraint_values)) {
				Model model = this.convertToPOJO(request);
				model = (Department) model;
				...
				JSONObject DBStatusResponse = DBUtil.getData("Department", "id",request.get("id"), getFields);
	...
	public Model convertToPOJO(JSONObject request) {
		...
		dept.setId(request.optString("id"));
		dept.setName(request.optString("name"));
		return dept;
	}
